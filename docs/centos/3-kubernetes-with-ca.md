...
> 大家可以看到`diff`只有一处，新的配置没有设定`api-server`。不访问`api-server`，它是怎么知道每个服务的`cluster ip`和pod的endpoints的呢？这就是因为`kubernetes`在启动每个服务service的时候会以环境变量的方式把所有服务的`ip`，端口等信息注入进来。
